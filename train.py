...
def worker(weights, arglist, seed, train_mode_int=1, max_len=-1):

  train_mode = (train_mode_int == 1)
  model.set_model_params(weights)
  reward_list, t_list = simulate(model, arglist,
...
def slave(arglist):
  ...
  while 1:
    ...
    for solution in solutions:
      ...
      jobidx = int(jobidx)
      seed = int(seed)
      fitness, timesteps = worker(weights, arglist, seed, train_mode, max_len)
...
if __name__ == "__main__":
  ...
  parser.add_argument("--recording_mode", type = bool, default = False, help = "training model")
